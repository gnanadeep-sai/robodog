"""
Analytic (closed-form) leg inverse-kinematics solver.

Straight relocation of the legacy `pupper/Kinematics.py` per the
migration blueprint (`src/Kinematics.py` -> `core/`). The math is
purely functional and untouched -- no `rclpy` imports, so this stays
independently unit-testable, same as the rest of `core/`.

`controller.Controller` is handed `four_legs_inverse_kinematics` as its
`inverse_kinematics` callable (see `nodes/controller_node.py`), exactly
as `run_robot_x.py` did for the legacy `Controller`.
"""
import numpy as np
