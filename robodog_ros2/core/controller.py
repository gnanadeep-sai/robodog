...
class Controller:
    ...
    def run(self, state, command):
        ...
        if command.activate_event:
            state.behavior_state = self.activate_transition_mapping.get(
                state.behavior_state, state.behavior_state
            )
        elif command.trot_event:
            state.behavior_state = self.trot_transition_mapping.get(
                state.behavior_state, state.behavior_state
            )
        elif command.hop_event:
            state.behavior_state = self.hop_transition_mapping.get(
                state.behavior_state, state.behavior_state
            )
        ...
        if state.behavior_state == BehaviorState.TROT:
            ...
            state.foot_locations = (
                self.config.default_stance + np.array([0, 0, command.height])[:, np.newaxis]
            )
            # Apply the desired body rotation
            rotated_foot_locations = (
                euler2mat(
    ...
    def set_pose_to_default(self, state):
        """Reset foot locations/joint angles to the default stance height.

        NOTE: the legacy implementation referenced undefined `state` and
        `controller` globals and would have raised a NameError if called;
        this version takes `state` explicitly so it is actually usable.
        """
        state.foot_locations = (
